...
Public Class cls_estados
    Dim var_Nombre_Tabla As String = "tbl_estados"
    Dim var_Campo_Id As String = "id"
    Dim var_Campo_Validacion As String = "nombre"
    Dim var_Campos As String = "nombre,id_pais,id_usuario_reg,fecha_reg"
    ...
    Public Property nombre() As String
        ...
        End Set
    End Property
    'Insertado por Patrick desde la clase base que fue tomada de cls_estados, porque en la tabla se encuentran los campos id, nombre, id_pais, id_reg,... etc
    Public Property id_pais() As Integer
        Get
            Return Me.var_id_pais
        End Get
        Set(ByVal value As Integer)
            Me.var_id_pais = value
        End Set
    End Property
    ...
    Public Sub Cargar(ByVal var_id_int As Integer)
        ...
        If obj_dt_int.Rows.Count > 0 Then
            ...
            Me.var_id_pais = ac_Funciones.formato_Numero(obj_dt_int.Rows(0).Item("id_pais").ToString)
    ...
    Public Function Actualizar(ByRef var_Error As String) As Boolean
        ...
        If Me.var_id = 0 Then   'NUEVO
            If Not Ingresar(Me.obj_Conex_int, Me.var_Nombre_Tabla, Me.var_Campos, Sql_Texto(Me.var_nombre) & "," & Sql_Texto(Me.var_id_pais) & "," & Sql_Texto(Me.var_id_usuario_reg) & "," & Sql_Texto(Me.var_fecha_reg), var_Error) Then
                Return False
                Exit Function
            End If
        ...
        ElseIf Me.var_id > 0 Then 'EDICION
            If Not ac_Funciones.Actualizar(Me.obj_Conex_int, Me.var_Nombre_Tabla, "nombre=" & Sql_Texto(Me.var_nombre) & ", id_pais=" & Sql_Texto(Me.var_id_pais), Me.var_Campo_Id & "=" & Me.var_id) Then
                Return False
                Exit Function
            End If
            Return True
    ...
    Public Shared Function Anular(ByVal var_id As Integer, ByVal var_id_usuario As Integer, ByRef var_mensaje As String) As Boolean
        ...
        Dim obj_estado As New cls_estados(var_id)
        If Not ac_Funciones.Actualizar(obj_Conex_int, cls_estados.Nombre_Tabla, "anulado= case when isnull(anulado,0)=0 then '1' else '0' end, fecha_anulacion=" & Sql_Texto(Now, True) & ", id_usuario_anulacion=" & var_id_usuario, cls_estados.Campo_Id & "=" & var_id, var_error) Then
            var_mensaje = var_error

            Dim obj_log As New cls_logs
            obj_log.ComentarioLog = "Error anulando el Estado '" & obj_estado.Id & " - " & obj_estado.nombre & "': " & var_error
            obj_log.FechaLog = Now
            obj_log.id_Menu = New cls_modulos("estados").Id
        ...
        Else
            ...
            Dim obj_log As New cls_logs
            obj_log.ComentarioLog = "Estado anulado '" & obj_estado.Id & " - " & obj_estado.nombre
            obj_log.FechaLog = Now
            obj_log.id_Menu = New cls_modulos("estados").Id
            obj_log.id_Usuario = var_id_usuario
            obj_log.idAccion = New cls_acciones("anular").Id
    ...
    Public Shared Function Eliminar(ByVal var_id As Integer, ByVal obj_usuario As cls_usuarios, ByRef var_mensaje As String) As Boolean
        ...
        If ac_Funciones.Eliminar(obj_conex, cls_estados.Nombre_Tabla, cls_estados.Campo_Id & "=" & var_id) < 0 Then
            var_mensaje = var_error
            Return False
        Else
            Return True
    ...
    Public Shared Function Lista(Optional ByVal var_filtro As String = "") As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim obj_dt_int As DataTable = Abrir_Tabla(obj_Conex_int, "select " & cls_estados.Campo_Id & " as id, " & cls_estados.Campo_Validacion & " as des from " & cls_estados.Nombre_Tabla & IIf(var_filtro <> "", " where " & var_filtro, "") & " order by " & cls_estados.Campo_Validacion & "")
    ...
    Public Shared Function ConsultaActivos(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_estados.ListadoActivos & "()"
    ...
    Public Shared Function ConsultaAnulados(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_estados.ListadoAnulados & "()"
        Dim var_msj As String = ""
    ...
    Public Shared Function Consulta(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_estados.Listado & "()"
        Dim var_msj As String = ""
