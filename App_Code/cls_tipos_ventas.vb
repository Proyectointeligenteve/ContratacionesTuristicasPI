...
Public Class cls_tipos_ventas
    Dim var_Nombre_Tabla As String = "tbl_tipos_ventas"
    ...
    Public Function Actualizar(ByRef var_msj As String, ByVal var_id_usuario As Integer) As Boolean
        If Validar_Existe(Me.obj_Conex_int, Me.var_Nombre_Tabla, Me.var_Campo_Validacion, Me.var_nombre, Me.var_Campo_Id, Me.var_id) Then
            If var_msj = "" Then
                var_msj = "El tipo venta '" & Me.var_nombre & "' ya existe en la base de datos"
            End If
            Return False
            Exit Function
        End If
        ...
        If Me.var_id = 0 Then   'NUEVO
            If Not Ingresar(Me.obj_Conex_int, Me.var_Nombre_Tabla, Me.var_Campos, Sql_Texto(Me.var_nombre) & "," & Sql_Texto(Me.var_id_usuario_reg) & "," & Sql_Texto(Me.var_fecha_reg), var_Error) Then
                ...
                obj_log.ComentarioLog = "Error agregando tipo_venta '" & Me.var_nombre & "': " & var_Error
                obj_log.FechaLog = Now
                obj_log.id_Menu = New cls_modulos("tiposventas").Id
        ...
        ElseIf Me.var_id > 0 Then 'EDICION
            If Not ac_Funciones.Actualizar(Me.obj_Conex_int, Me.var_Nombre_Tabla, "nombre=" & Sql_Texto(Me.var_nombre), Me.var_Campo_Id & "=" & Me.var_id) Then
                ...
                Dim obj_log As New cls_logs
                obj_log.ComentarioLog = "Error editando tipo_venta '" & Me.var_id & " - " & Me.var_nombre & "': " & var_error
                obj_log.FechaLog = Now
                obj_log.id_Menu = New cls_modulos("tiposventas").Id
            ...
            Else
                ...
                Dim obj_log As New cls_logs
                obj_log.ComentarioLog = "tipo venta editada: {" & obj_sb.ToString & "}"
                obj_log.FechaLog = Now
                obj_log.id_Menu = New cls_modulos("tiposventas").Id
                obj_log.id_Usuario = var_id_usuario_reg
                obj_log.idAccion = New cls_acciones("editar").Id
    ...
    Public Shared Function Anular(ByVal var_id As Integer, ByVal var_id_usuario As Integer, ByRef var_mensaje As String) As Boolean
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_error As String = ""
        Dim obj_tipo_venta As New cls_tipos_ventas(var_id)
        If Not ac_Funciones.Actualizar(obj_Conex_int, cls_tipos_ventas.Nombre_Tabla, "anulado= case when isnull(anulado,0)=0 then '1' else '0' end, fecha_anulacion=" & Sql_Texto(Now, True) & ", id_usuario_anulacion=" & var_id_usuario, cls_tipos_ventas.Campo_Id & "=" & var_id, var_error) Then
            var_mensaje = var_error

            Dim obj_log As New cls_logs
            obj_log.ComentarioLog = "Error anulando tipo_venta '" & obj_tipo_venta.Id & " - " & obj_tipo_venta.nombre & "': " & var_error
            obj_log.FechaLog = Now
            obj_log.id_Menu = New cls_modulos("tiposventas").Id
            obj_log.id_Usuario = var_id_usuario
        ...
        Else
            ...
            Dim obj_log As New cls_logs
            obj_log.ComentarioLog = "tipo_venta anulado '" & obj_tipo_venta.Id & " - " & obj_tipo_venta.nombre
            obj_log.FechaLog = Now
            obj_log.id_Menu = New cls_modulos("tipoventa").Id
            obj_log.id_Usuario = var_id_usuario
            obj_log.idAccion = New cls_acciones("anular").Id
            obj_log.ResultadoLog = True
            obj_log.InsertarLog()
    ...
    Public Shared Function Eliminar(ByVal var_id As Integer, ByVal obj_usuario As cls_usuarios, ByRef var_mensaje As String) As Boolean
        ...
        If ac_Funciones.Eliminar(obj_conex, cls_tipos_ventas.Nombre_Tabla, cls_tipos_ventas.Campo_Id & "=" & var_id) < 0 Then
            var_mensaje = var_error
            Return False
        Else
            Return True
    ...
    Public Shared Function Lista(Optional ByVal var_filtro As String = "") As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim obj_dt_int As DataTable = Abrir_Tabla(obj_Conex_int, "select " & cls_tipos_ventas.Campo_Id & " as id, " & cls_tipos_ventas.Campo_Validacion & " as des from " & cls_tipos_ventas.Nombre_Tabla & IIf(var_filtro <> "", " where " & var_filtro, "") & " order by " & cls_tipos_ventas.Campo_Validacion & "")
    ...
    Public Shared Function ConsultaActivos(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_tipos_ventas.ListadoActivos & "()"
    ...
    Public Shared Function ConsultaAnulados(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_tipos_ventas.ListadoAnulados & "()"
        Dim var_msj As String = ""
    ...
    Public Shared Function Consulta(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_tipos_ventas.Listado & "()"
        Dim var_msj As String = ""
