...
Public Class cls_aerolineas
    ...
    Public Shared Function SiguienteNumero() As Integer
        ...
        Return ac_Funciones.formato_Numero(ac_Funciones.Valor_De(obj_Connection, "select isnull(max(right(num,4)),0)from (select case when cast(right(codigo,4) as int)=1 then cast(right(codigo,4) as int) else 0 end as num from tbl_aerolineas) as c").ToString)
    End Function
End Class
