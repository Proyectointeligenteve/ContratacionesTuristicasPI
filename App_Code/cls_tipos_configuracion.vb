...
Public Class cls_tipos_configuracion
    Dim var_Nombre_Tabla As String = "tbl_tipos_configuracion"
    ...
    Public Property nombre() As String
        ...
        End Set
    End Property
    Public Property id_usuario_registro() As Integer
        Get
    ...
    Public Sub Cargar(ByVal var_id_int As Integer)
        ...
        If obj_dt_int.Rows.Count > 0 Then
            ...
            Me.var_id_usuario_registro = formato_Numero(obj_dt_int.Rows(0).Item("id_usuario_registro").ToString)
            Me.var_fecha_registro = formato_Fecha(obj_dt_int.Rows(0).Item("fecha_registro").ToString)
        Else
            Me.var_id = 0
        End If
    End Sub
    ...
    Public Function Actualizar(ByRef var_msj As String, ByVal var_id_usuario As Integer) As Boolean
        If Validar_Existe(Me.obj_Conex_int, Me.var_Nombre_Tabla, Me.var_Campo_Validacion, Me.var_nombre, Me.var_Campo_Id, Me.var_id) Then
            If var_msj = "" Then
                var_msj = "El tipo_configuracion '" & Me.var_nombre & "' ya existe en la base de datos"
            End If
            Return False
            Exit Function
        End If
        ...
        If Me.var_id = 0 Then   'NUEVO
            If Not Ingresar(Me.obj_Conex_int, Me.var_Nombre_Tabla, Me.var_Campos, Sql_Texto(Me.var_nombre) & "," & Sql_Texto(Me.var_id_usuario_registro) & "," & Sql_Texto(Me.var_fecha_registro), var_Error) Then
                ...
                obj_log.ComentarioLog = "Error agregando tipo_configuracion '" & Me.var_nombre & "': " & var_Error
                obj_log.FechaLog = Now
                obj_log.id_Menu = New cls_modulos("tipos_configuracion").Id
        ...
        ElseIf Me.var_id > 0 Then 'EDICION
            If Not ac_Funciones.Actualizar(Me.obj_Conex_int, Me.var_Nombre_Tabla, "nombre=" & Sql_Texto(Me.var_nombre), Me.var_Campo_Id & "=" & Me.var_id) Then
                ...
                Dim obj_log As New cls_logs
                obj_log.ComentarioLog = "Error editando tipo_configuracion '" & Me.var_id & " - " & Me.var_nombre & "': " & var_error
                obj_log.FechaLog = Now
                obj_log.id_Menu = New cls_modulos("tipos_configuracion").Id
            ...
            Else
                ...
                Dim obj_log As New cls_logs
                obj_log.ComentarioLog = "tipo_configuracion editada: {" & obj_sb.ToString & "}"
                obj_log.FechaLog = Now
                obj_log.id_Menu = New cls_modulos("tipos_configuracion").Id
                obj_log.id_Usuario = var_id_usuario_registro
                obj_log.idAccion = New cls_acciones("editar").Id
    ...
    Public Shared Function Anular(ByVal var_id As Integer, ByVal var_id_usuario As Integer, ByRef var_mensaje As String) As Boolean
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_error As String = ""
        Dim obj_tipo_configuracion As New cls_tipos_configuracion(var_id)
        If Not ac_Funciones.Actualizar(obj_Conex_int, cls_tipos_configuracion.Nombre_Tabla, "anulado= case when isnull(anulado,0)=0 then '1' else '0' end, fecha_anulacion=" & Sql_Texto(Now, True) & ", id_usuario_anulacion=" & var_id_usuario, cls_tipos_configuracion.Campo_Id & "=" & var_id, var_error) Then
            var_mensaje = var_error

            Dim obj_log As New cls_logs
            obj_log.ComentarioLog = "Error anulando tipo_configuracion '" & obj_tipo_configuracion.Id & " - " & obj_tipo_configuracion.nombre & "': " & var_error
            obj_log.FechaLog = Now
            obj_log.id_Menu = New cls_modulos("tipo_configuracion").Id
            obj_log.id_Usuario = var_id_usuario
        ...
        Else
            ...
            Dim obj_log As New cls_logs
            obj_log.ComentarioLog = "tipo_configuracion anulado '" & obj_tipo_configuracion.Id & " - " & obj_tipo_configuracion.nombre
            obj_log.FechaLog = Now
            obj_log.id_Menu = New cls_modulos("tipo_configuracion").Id
            obj_log.id_Usuario = var_id_usuario
            obj_log.idAccion = New cls_acciones("anular").Id
    ...
    Public Shared Function Eliminar(ByVal var_id As Integer, ByVal obj_usuario As cls_usuarios, ByRef var_mensaje As String) As Boolean
        ...
        If ac_Funciones.Eliminar(obj_conex, cls_tipos_configuracion.Nombre_Tabla, cls_tipos_configuracion.Campo_Id & "=" & var_id) < 0 Then
            var_mensaje = var_error
            Return False
        Else
            Return True
    ...
    Public Shared Function Lista(Optional ByVal var_filtro As String = "") As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim obj_dt_int As DataTable = Abrir_Tabla(obj_Conex_int, "select " & cls_tipos_configuracion.Campo_Id & " as id, " & cls_tipos_configuracion.Campo_Validacion & " as des from " & cls_tipos_configuracion.Nombre_Tabla & IIf(var_filtro <> "", " where " & var_filtro, "") & " order by " & cls_tipos_configuracion.Campo_Validacion & "")
    ...
    Public Shared Function ConsultaActivos(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_tipos_configuracion.ListadoActivos & "()"
    ...
    Public Shared Function ConsultaAnulados(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_tipos_configuracion.ListadoAnulados & "()"
        Dim var_msj As String = ""
    ...
    Public Shared Function Consulta(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_tipos_configuracion.Listado & "()"
        Dim var_msj As String = ""
