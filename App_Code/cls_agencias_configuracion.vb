...
Public Class cls_agencias_configuracion
    Dim var_id_agencia_Tabla As String = "tbl_agencias_configuracion"
    Dim var_Campo_Id As String = "id"
    Dim var_Campo_Validacion As String = "nombre"
    Dim var_Campos As String = "id_agencia,id_tipo,id_aerolinea,id_hotel,id_vehiculo,comision,forma"
    ...
    Public Shared ReadOnly Property Nombre_Tabla() As String
        Get
            Return "tbl_agencias_configuracion"
        End Get
    End Property
    ...
    Public Sub Cargar(ByVal var_id_int As Integer)
        ...
        obj_dt_int = Abrir_Tabla(Me.obj_Conex_int, "Select * from " & Me.var_id_agencia_Tabla & " WHERE " & Me.var_Campo_Id & "=" & var_id_int)
        If obj_dt_int.Rows.Count > 0 Then
            Me.var_id = ac_Funciones.formato_Numero(obj_dt_int.Rows(0).Item("id").ToString)
            Me.var_id_agencia = ac_Funciones.formato_Numero(obj_dt_int.Rows(0).Item("id_agencia").ToString)
    ...
    Public Function Actualizar(ByRef var_Error As String) As Boolean
        If Validar_Existe(Me.obj_Conex_int, Me.var_id_agencia_Tabla, Me.var_Campo_Validacion, Me.var_id_agencia, Me.var_Campo_Id, Me.var_id) Then
            If var_Error = "" Then
                var_Error = "El agencia_configuracion '" & Me.var_id_agencia & "' ya existe en la base de datos"
            End If
            Return False
            Exit Function
        End If
        ...
        If Me.var_id = 0 Then   'NUEVO
            If Not Ingresar(Me.obj_Conex_int, Me.var_id_agencia_Tabla, Me.var_Campos, Sql_Texto(Me.var_id_agencia) & "," & Sql_Texto(Me.var_id_tipo) & "," & Sql_Texto(Me.var_id_aerolinea.Id) & "," & Sql_Texto(Me.var_id_hotel.Id) & "," & Sql_Texto(Me.var_id_vehiculo.Id) & "," & Sql_Texto(Me.var_comision) & "," & Sql_Texto(Me.var_forma), var_Error) Then
                Return False
                Exit Function
            End If
            Me.var_id = Valor_De(Me.obj_Conex_int, "select " & Me.var_Campo_Id & " from " & Me.var_id_agencia_Tabla & " where id_agencia=" & Sql_Texto(Me.var_id_agencia))
            Return True
        ElseIf Me.var_id > 0 Then 'EDICION
            If Not ac_Funciones.Actualizar(Me.obj_Conex_int, Me.var_id_agencia_Tabla, "id_agencia=" & Sql_Texto(Me.var_id_agencia) & ", id_tipo=" & Sql_Texto(Me.var_id_tipo) & ",id_aerolinea=" & Sql_Texto(Me.var_id_aerolinea.Id) & ",id_hotel=" & Sql_Texto(Me.var_id_hotel.Id) & ",id_vehiculo=" & Sql_Texto(Me.var_id_vehiculo) & ",comision=" & Sql_Texto(Me.var_comision) & ",forma=" & Sql_Texto(Me.var_forma), Me.var_Campo_Id & "=" & Me.var_id) Then
                Return False
                Exit Function
            End If
            Return True
    ...
    Public Shared Function Eliminar(ByVal var_id As Integer, ByVal obj_usuario As cls_usuarios, ByRef var_mensaje As String) As Boolean
        ...
        If ac_Funciones.Eliminar(obj_conex, cls_agencias_configuracion.Nombre_Tabla, cls_agencias_configuracion.Campo_Id & "=" & var_id) < 0 Then
    ...
    Public Shared Function Lista(Optional ByVal var_filtro As String = "") As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim obj_dt_int As DataTable = Abrir_Tabla(obj_Conex_int, "select " & cls_agencias_configuracion.Campo_Id & " as id, " & cls_agencias_configuracion.Campo_Validacion & " as des from " & cls_agencias_configuracion.Nombre_Tabla & IIf(var_filtro <> "", " where " & var_filtro, "") & " order by " & cls_agencias_configuracion.Campo_Validacion & "")
    ...
    Public Shared Function Consulta(ByRef var_error As String) As DataTable
        Dim obj_Conex_int As New SqlConnection(ConfigurationManager.ConnectionStrings("connection").ConnectionString)
        Dim var_consulta As String = "Select * from " & cls_agencias_configuracion.Listado & "()"
